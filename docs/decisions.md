...
- **`firestore.rules` is written, validated, and committed but NOT deployed to either live project yet** — needs explicit user go-ahead (`yarn deploy-rules-dev` then `yarn deploy-rules-prod`), especially given the admin self-promotion hole is presumably still live in production until the prod deploy actually runs.
